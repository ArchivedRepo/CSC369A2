...
void initSafeTrafficLight(SafeTrafficLight* light, int horizontal, int vertical) {
	...
	// Initialize the mutexes and cond variables for enter/exit lanes
	...
	// Initialize the mutexes and cond variables for enter/move through intersections
...
void runTrafficLightCar(Car* car, SafeTrafficLight* light) {
	...
	// signal left-turn cars if a car going straight exits the intersection
	if(car->action == STRAIGHT){
		condBroadcast(&light->leftTurnCond);
	}
	// signal cars waiting for entering since traffic light signal might have changed
